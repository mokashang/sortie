...
- 账号邮件(验证 / 重置密码 / 改邮箱 / 删除账号,`src/lib/mailer.ts`)的主题、正文、按钮都在 `mail` 命名空间;`src/lib/auth.ts` 的发信钩子从触发请求的 cookie 取语言(`mailLang(request)`),没有请求就用 `serverLang()`。`tests/mail-i18n.test.ts` 断言英文邮件不含中文且两种语言都带链接。
