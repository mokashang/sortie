...
describe("runLogLines (full run log for the App's 详情 view)", () => {
  ...
  it("returns every line of the run's log, in order, for running and finished runs alike", () => {
    const run = startExecutor(db, U, "apply", { plan: [{ direction: "swe_general", count: 1 }] }, { logDir: tmpLogDir }, "user_chrome");
    expect(runLogLines(db, U, run.id)).toEqual([]);
    for (let i = 1; i <= 40; i++) appendRunLog(db, U, run.id, `step ${i}`);
    const lines = runLogLines(db, U, run.id);
    expect(lines).toHaveLength(40);
    expect(lines[0]).toMatch(/\] step 1$/);
    expect(lines[39]).toMatch(/\] step 40$/);

    finishRun(db, U, run.id, "done", "ok");
    expect(runLogLines(db, U, run.id)).toHaveLength(40);
  });

  it("throws for an unknown run and returns [] when the log file is gone", () => {
    expect(() => runLogLines(db, U, 999)).toThrow();
    const run = startExecutor(db, U, "apply", {}, { logDir: tmpLogDir }, "user_chrome");
