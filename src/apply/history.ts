...
export function setStage(db: DB, jobId: number, stage: string, note?: string | null): void {
  ...
  db.prepare("UPDATE applications SET status = ? WHERE job_id = ?").run(stage, jobId);
...
export function applicationHistory(db: DB): HistoryRow[] {
  const rows = db
    .prepare(
...
export function todaySubmitted(db: DB): TodaySubmittedRow[] {
  const rows = db
    .prepare(
...
export function recordExternalSubmission(db: DB, jobId: number, note: string): void {
  const row = db.prepare("SELECT status FROM applications WHERE job_id = ?").get(jobId) as
    | { status: string }
    | undefined;
  ...
  db.prepare(
    `UPDATE applications SET status = 'submitted', submitted_at = datetime('now'), confirm_decision = NULL,
            needs_manual_reason = NULL, pending_questions = NULL WHERE job_id = ?`
  ).run(jobId);
  logEvent(db, "application_stage", {
    entity: "application",
    entityId: jobId,
...
// drops the form.
export function archiveManual(db: DB, jobIds: number[]): ArchiveManualResult {
  const stmt = db.prepare(
    `UPDATE applications SET status = 'archived', confirm_decision = NULL,
     ...
     WHERE job_id = ? AND ((status = 'matched' AND needs_manual_reason IS NOT NULL) OR status = 'needs_info')`
  );
  const skipped: number[] = [];
  let archived = 0;
  const run = db.transaction((ids: number[]) => {
    for (const id of ids) {
      if (stmt.run(id).changes === 1) archived++;
