...
export const POST = withUser(async (req, { userId, via }) => {
  try {
    ...
    if (via !== "session") return NextResponse.json({ error: t.tokenCreateInBrowser }, { status: 403 });
    const body = await req.json().catch(() => ({}));
    const name = String(body.name ?? "").trim() || t.defaultTokenName;
...
export const DELETE = withUser(async (req, { userId, via }) => {
  if (via !== "session") return NextResponse.json({ error: messagesFor(langFromRequest(req)).errors.tokenRevokeInBrowser }, { status: 403 });
