...
export default async function QueuePage({
  ...
  const scoredTotal = count("SELECT COUNT(*) n FROM matches WHERE user_id = ?", user.id);
  const matchedTotal = count(
    `SELECT COUNT(*) n FROM applications a JOIN jobs j ON j.id = a.job_id
     WHERE a.user_id = ? AND a.status='matched' AND ${QUEUE_ELIGIBLE_SQL}`,
    user.id
  );

  const tabs = queueByDirection(db, user.id);
  ...
  const result = isAllTab
    ...
    : pagedQueue(db, user.id, { direction, page, pageSize: PAGE_SIZE, sort, mode: mode === "all" ? undefined : mode, q: q || undefined });
