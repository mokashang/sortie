  ...
  const rows = db
    .prepare(
      `SELECT a.job_id, j.company, j.title, j.apply_url, m.direction, a.status, a.needs_manual_reason,
       ...
       WHERE (a.status = 'needs_info' AND a.pending_questions IS NOT NULL)
...
export function resolveLogin(db: DB, host: string): ResolveLoginResult {
  ...
  const rows = db
    ...
    .all() as { job_id: number; pending_questions: string }[];
  ...
  db.transaction(() => {
    for (const r of rows) {
      ...
      if (remaining.length === 0) {
        db.prepare("UPDATE applications SET needs_manual_reason = NULL, pending_questions = NULL WHERE job_id = ?").run(r.job_id);
        cleared.push(r.job_id);
      } else {
        db.prepare("UPDATE applications SET needs_manual_reason = ?, pending_questions = ? WHERE job_id = ?").run(
  ...
  logEvent(db, "application_login_done", { payload: { host: h, jobIds: cleared, touched } });
