...
**关键决定(用户 2026-08-30 明确)**:经历内容通过 UI 由用户自己填写(像网申系统的 Profile 板块),**不导入用户现有材料、不在建系统时向用户索取经历细节**。系统先把空的结构化 Profile UI + 生成引擎骨架建好,用户之后自己在 UI 里录入经历。经历内容不可泛化——这正是 Phase B 让任何用户填自己经历即可用的地基。

- **经历素材库(素材层)**:数据库里的结构化经历——每段教育/实习/项目/研究/技能作为一条条目,每条含标题、机构、时间、若干 bullet;bullet 可标注方向相关度(同一个项目对 AI Infra 强调分布式训练、对 Embedded 强调实时约束)。全部经用户在 UI 录入
- **生成层**:Claude 按目标方向从素材库选条目/选 bullet、组版 → 套 LaTeX 模板 → 用 tectonic 编译 PDF
- **UI(Profile 录入页 + Resume Studio 页)**:Profile 页像网申 Profile 一样增删改经历条目;Studio 页选方向一键生成、预览 PDF、逐条编辑、版本对比;定稿版本进入简历库供匹配引擎选用
- **明确不做 per-JD 微调**(用户判断:工作量大、边际收益低)。每岗位由 Claude 从定稿库中选最契合版本
- **工具链**:tectonic(自包含 LaTeX,已装 `/opt/homebrew/bin/tectonic`);可选回退 pdflatex
