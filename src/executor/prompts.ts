...
2. **上线页面最终资格检查 + 打开并填表**:
   ...
   - **登录墙**:回报 \`${C} -X POST ${APP_BASE}/api/apply/report -H 'content-type: application/json' -d '{"jobId": <jobId>, "status": "needs_manual", "reason": "${LOGIN_WALL_REASON}"}'\`,关掉 tab,继续下一轮。
...
3. **回报填表结果**(用 §2 第 2 步快照读回的**实际**字段值,不是你打算填的值):
   ...
   - 遇到 §3 needs_manual 触发条件(见下方列表,含中途才发现的登录墙/CAPTCHA/视频题等):\`${C} -X POST ${APP_BASE}/api/apply/report -H 'content-type: application/json' -d '{"jobId": <jobId>, "status": "needs_manual", "reason": "..."}'\`,\`mcp__playwright__browser_tabs\`(action: close)关掉这个 tab,继续下一轮。
