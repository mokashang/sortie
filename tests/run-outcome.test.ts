...
describe("run outcome end to end", () => {
  ...
  function startPlanRun(plan: { direction: string; count: number; mode: "direct" | "referral" }[]): number {
    const { id } = startExecutor(db, "apply", { plan }, { logDir }, "user_chrome");
  ...
  it("stamps run_id on claimed jobs and settles 未完成 with the counts when the plan is not met", () => {
    ...
    expect(currentApplyRunId(db)).toBe(runId);

    // a: filled, confirmed, submitted.
    expect((takeNextApplication(db, testProfile(), { direction: "swe_general" }) as { jobId: number }).jobId).toBe(a);
    expect(runIdOf(db, a)).toBe(runId);
    reportFill(db, { jobId: a, status: "awaiting_confirm", filledFields: { Name: "Mengjia Shang" } });
    decide(db, a, "approve");
    reportSubmitted(db, a);
    // b: dead link → closed (archived without a card).
    expect((takeNextApplication(db, testProfile(), { direction: "swe_general" }) as { jobId: number }).jobId).toBe(b);
    reportFill(db, { jobId: b, status: "closed", reason: "dead link" });
    // c: filled, still waiting for the user when the run ends.
    expect((takeNextApplication(db, testProfile(), { direction: "swe_general" }) as { jobId: number }).jobId).toBe(c);
    reportFill(db, { jobId: c, status: "awaiting_confirm", filledFields: { Name: "Mengjia Shang" } });

    expect(storedOutcome(db, runId)).toBeNull(); // nothing settled while the run is live
    finishRun(db, runId, "done", "submitted 1, filled 1 more");
    ...
    const row = executorStatus(db).find((r) => r.id === runId)!;
    expect(row.outcome).toEqual(outcome);
    expect(currentApplyRunId(db)).toBeNull();
  });
  ...
  it("is 已完成 when every planned application was filled, even if some are still awaiting confirmation", () => {
    ...
    for (const id of [a, b]) {
      expect((takeNextApplication(db, testProfile(), { direction: "swe_general" }) as { jobId: number }).jobId).toBe(id);
      reportFill(db, { jobId: id, status: "awaiting_confirm", filledFields: { Name: "Mengjia Shang" } });
    }
  ...
  it("a user-rejected fill still counts as filled (the fill happened) but shows as a 待处理 card", () => {
    ...
    takeNextApplication(db, testProfile(), { direction: "swe_general" });
    reportFill(db, { jobId: a, status: "awaiting_confirm", filledFields: { Name: "Mengjia Shang" } });
    decide(db, a, "reject", "wrong resume");
    finishRun(db, runId, "done");
  ...
  it("a login wall or an executor error is a 待处理 card, never an achievement", () => {
    ...
    const runId = startPlanRun([{ direction: "swe_general", count: 2, mode: "direct" }]);
    takeNextApplication(db, testProfile(), { direction: "swe_general" });
    reportFill(db, { jobId: a, status: "needs_info", questions: [{ kind: "login", key: "x", label: "登录一次", host: "a.example" }] });
    takeNextApplication(db, testProfile(), { direction: "swe_general" });
    reportFill(db, { jobId: b, status: "error", reason: "tab crashed" });
    finishRun(db, runId, "done");
  ...
  it("referral entries: a company with someone to contact counts, a 找不到人 company does not", () => {
    ...
    const first = takeNextReferral(db, { direction: "swe_general" }) as { company: string; jobs: { jobId: number }[] };
    expect(first.company).toBe("Tradeweb");
    expect(runIdOf(db, e)).toBe(runId);
    reportNoContact(db, [e], "LinkedIn free invite quota exhausted");
    const second = takeNextReferral(db, { direction: "swe_general" }) as { company: string };
  ...
  it("runs with no plan (resume-only, other kinds) get no outcome and keep the plain 已完成", () => {
    const { id } = startExecutor(db, "apply", { resume: true }, { logDir }, "user_chrome");
    claimNextRun(db, "user_chrome");
    finishRun(db, id, "done");
    expect(storedOutcome(db, id)).toBeNull();
    expect(runStatusDisplay("done", null)).toEqual({ label: "已完成", tone: "good" });

    const check = startExecutor(db, "referral_check", {}, { logDir }, "user_chrome");
    claimNextRun(db, "user_chrome");
    finishRun(db, check.id, "done");
  ...
  it("a job claimed with no apply run live is not stamped, and a later run only counts its own rows", () => {
    ...
    const runId = startPlanRun([{ direction: "swe_general", count: 1, mode: "direct" }]);
    expect((takeNextApplication(db, testProfile(), { direction: "swe_general" }) as { jobId: number }).jobId).toBe(b);
    reportFill(db, { jobId: b, status: "awaiting_confirm", filledFields: { Name: "Mengjia Shang" } });
    finishRun(db, runId, "done");
