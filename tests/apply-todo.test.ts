...
describe("login walls", () => {
  it("a login item pauses the application right away (the assistant moves on) and shows as a card", () => {
    ...
    reportFill(db, { jobId, status: "needs_info", questions: [LOGIN] });
    ...
    expect(confirmStatus(db, jobId).status).toBe("matched");
    const cards = pendingInfo(db);
    expect(cards).toHaveLength(1);
    expect(cards[0]).toMatchObject({ jobId, status: "matched", questions: [expect.objectContaining({ kind: "login" })] });
    expect(openLoginWalls(db).get("jobs.apple.com")).toMatchObject({ label: LOGIN.label });
  });
  ...
  it("the picker pauses other jobs on the same host with the same item instead of handing them out", () => {
    ...
    reportFill(db, { jobId: walled, status: "needs_info", questions: [LOGIN] });
    ...
    const task = takeNextApplication(db, profile) as ApplyTask;
    ...
    expect([...openLoginWalls(db).keys()]).toEqual(["jobs.apple.com"]);
  });
  ...
  it("我登好了 clears every paused job on that host so they can be re-queued, and leaves other items waiting", () => {
    ...
    reportFill(db, { jobId: a, status: "needs_info", questions: [LOGIN] });
    reportFill(db, { jobId: b, status: "needs_info", questions: [LOGIN, { key: "gpa", label: "GPA" }] });
    reportFill(db, { jobId: c, status: "needs_info", questions: [{ ...LOGIN, host: "career-hcm20.ns2cloud.com", label: "注册 L3Harris 候选人账号" }] });

    const r = resolveLogin(db, "JOBS.APPLE.COM");
    ...
    expect(openLoginWalls(db).has("jobs.apple.com")).toBe(false);
    expect(() => resolveLogin(db, "  ")).toThrow();
  });
});
...
describe("files, actions and multi-select answers", () => {
  it("a file answer is stored on the application but never remembered as a standard answer", () => {
    ...
    const r = answerInfo(db, jobId, { transcript: { value: "E:/sortie/data/documents/transcript.pdf" }, gpa: { value: "3.9" } }, (p) => (persisted = p));
    expect(r).toEqual({ remembered: { gpa: "3.9" }, status: "prepared" });
    expect(persisted).toEqual({ gpa: "3.9" });
    expect(confirmStatus(db, jobId).infoAnswers).toEqual({ transcript: "E:/sortie/data/documents/transcript.pdf", gpa: "3.9" });
  });

  it("an action item is answered with 'done' and never remembered; a required one can't be skipped", () => {
    const db = openDb(":memory:");
    const jobId = seed(db);
    reportFill(db, { jobId, status: "needs_info", questions: [{ key: "captcha", label: "在打开的标签页里完成人机验证", kind: "action" }] });
    expect(() => answerInfo(db, jobId, {}, () => {})).toThrow(/missing answer/);
    let persisted: Record<string, string> | null = null;
    const r = answerInfo(db, jobId, { captcha: { value: "done" } }, (p) => (persisted = p));
  ...
  it("a multi-select answer must be a subset of the options and is normalized to the option order", () => {
    const db = openDb(":memory:");
    const jobId = seed(db);
    reportFill(db, { jobId, status: "needs_info", questions: [{ key: "tech_stacks", label: "Stacks", options: ["Python", "Go", "Rust"], multiple: true }] });
    expect(() => answerInfo(db, jobId, { tech_stacks: { value: "Python; COBOL" } }, () => {})).toThrow(/not one of the options/);
    const r = answerInfo(db, jobId, { tech_stacks: { value: "Rust;  Python " } }, () => {});
...
describe("cards that only the user can move", () => {
  it("a manual item pauses right away; 让助手再试一次 clears reason and items", () => {
    const db = openDb(":memory:");
    const jobId = seed(db);
    reportFill(db, { jobId, status: "needs_info", questions: [{ key: "video", label: "要录一段 60 秒视频", kind: "manual", url: "https://acme.example/video" }] });
    expect(app(db, jobId).status).toBe("matched");
    expect(pendingInfo(db)[0].questions[0]).toMatchObject({ kind: "manual", url: "https://acme.example/video" });
    unpark(db, jobId);
  ...
  it("an error report becomes a card with the reason as its hint", () => {
    const db = openDb(":memory:");
    const jobId = seed(db);
    reportFill(db, { jobId, status: "error", reason: "tab crashed twice" });
  ...
  it("a rejected fill becomes a card and the reason rides along into the next answer pack", () => {
    ...
    decide(db, jobId, "reject", "Boston was picked instead of Los Angeles");
    expect(items(db, jobId)).toEqual([{ key: "rejected", label: "你退回了这份填写", kind: "manual", hint: "Boston was picked instead of Los Angeles" }]);
    expect(pendingInfo(db).map((c) => c.jobId)).toEqual([jobId]);
    unpark(db, jobId);
    const task = takeNextApplication(db, profile, { jobIds: [jobId] }) as ApplyTask;
  ...
  it("a legacy needs_manual without items synthesizes one; with items (info timeout) the items survive", () => {
    const db = openDb(":memory:");
    const plain = seed(db);
    reportFill(db, { jobId: plain, status: "needs_manual", reason: "CAPTCHA on submit page" });
    expect(items(db, plain)).toEqual([{ key: "manual", label: "助手没能完成这份申请", kind: "manual", hint: "CAPTCHA on submit page" }]);

    const waiting = seed(db);
    reportFill(db, { jobId: waiting, status: "needs_info", questions: [{ key: "gpa", label: "GPA" }] });
    reportFill(db, { jobId: waiting, status: "needs_manual", reason: "info request timed out after 30 minutes" });
    ...
    expect(answerInfo(db, waiting, { gpa: { value: "3.8" } }, () => {}).status).toBe("matched");
  ...
  it("a paused row with only a reason (pre-design data) still renders as a manual card", () => {
    ...
    expect(pendingInfo(db)[0].questions).toEqual([{ key: "manual", label: "助手没能完成这份申请", kind: "manual", hint: "login wall: Apple Jobs" }]);
  });

  it("the picker pauses a job with no resume for its direction as a card pointing at the resumes tab", () => {
    const db = openDb(":memory:");
    const jobId = seed(db, { status: "matched" });
    expect(takeNextApplication(db, profile)).toEqual({ done: true });
  ...
  it("跳过这个岗 archives paused and waiting rows alike, and undo returns them to the queue clean", () => {
    ...
    reportFill(db, { jobId: paused, status: "error", reason: "x" });
    const clean = seed(db, { status: "matched" });
    expect(archiveManual(db, [waiting, paused, clean])).toEqual({ archived: 2, skipped: [clean] });
...
describe("handled without a card", () => {
  it("closed archives the application, marks the job's page closed and can mute the board", () => {
    ...
    reportFill(db, { jobId: gone, status: "closed", reason: "whole Ashby board returns Page not found", boardGone: true });
    ...
    reportFill(db, { jobId: single, status: "closed", reason: "posting no longer available" });
...
describe("answer pack extras", () => {
  it("carries the standing documents and the direction's experience highlights", () => {
    const db = openDb(":memory:");
    seedResume(db);
    createExperience(db, { kind: "project", title: "Sortie", organization: null, bullets: [{ text: "Built a job-application pipeline", directions: ["swe_general"] }], sort_order: 1 });
    createExperience(db, { kind: "skill", title: "Languages", organization: null, bullets: [{ text: "C++", directions: [] }], sort_order: 0 });
    seed(db, { status: "matched" });
    const task = takeNextApplication(db, profile, { documents: { transcript: "E:/sortie/data/documents/transcript.pdf" } }) as ApplyTask;
...
describe("the assistant that asked is gone (its run failed or was reaped)", () => {
  it("answering re-queues the job instead of handing it to nobody, and 让助手重新来 works from a waiting card", () => {
    const db = openDb(":memory:");
    const a = seed(db);
    reportFill(db, { jobId: a, status: "needs_info", questions: [{ key: "gpa", label: "GPA" }] });
    expect(answerInfo(db, a, { gpa: { value: "3.9" } }, () => {}, { executorWaiting: false }).status).toBe("matched");
    expect(app(db, a)).toMatchObject({ status: "matched", needs_manual_reason: null, pending_questions: null });
    expect(confirmStatus(db, a).infoAnswers).toEqual({ gpa: "3.9" });

    const b = seed(db);
    reportFill(db, { jobId: b, status: "needs_info", questions: [{ key: "gpa", label: "GPA" }] });
    unpark(db, b);
  ...
  it("with the executor still on the form, answering keeps the row for it (prepared)", () => {
    ...
    const a = seed(db);
    reportFill(db, { jobId: a, status: "needs_info", questions: [{ key: "gpa", label: "GPA" }] });
    expect(answerInfo(db, a, { gpa: { value: "3.9" } }, () => {}, { executorWaiting: true }).status).toBe("prepared");
  });
});
