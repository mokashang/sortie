...
export function buildAttendedPrompt(
  ...
  return [
    ...
    `回报 awaiting_confirm 之后不要等、不要轮询、不要 sleep 循环:填好的标签页保持打开,本段做满就 finish,然后再 GET claim-next 一次——还有排队的就接着做;没有就**直接停下来,什么都不做**(不要退出)。服务器会在需要时往这个终端打一行消息:\`[Sortie] approved job <id>\` = 用户批准了,回到你自己为它填的那个标签页(tabs_context_mcp 找到它),核对表单值仍与回报的 filledFields 一致后点 Submit,看到成功页 POST /api/apply/report {jobId,status:'submitted'};**绝不关标签页**(关掉一个标签页会让扩展销毁整个标签组,其他填好的表单一起消失——2026-09-17 Lumion 就是这样丢的),成功页留着或把那个标签页导航到 about:blank;\`[Sortie] rejected job <id>\` = 用户退回了,不提交,同样不关标签页、导航到 about:blank 即可;\`[Sortie] run <id> queued\` = 有新任务,GET claim-next 接单照常执行。每条消息处理完就再次停下等下一条。原因:每个会话只看得到自己标签组里的标签页,换一个会话就得重填、让用户再确认一次,所以由你自己一直守着这些标签页直到用户决定。会话空闲(没有任务、没有待确认的申请)15 分钟后服务器才会收掉它。`,
