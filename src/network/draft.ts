...
// Highlights (the candidate's "one concrete thing" material) moved to src/resume/highlights.ts so
// the apply answer pack can share them; re-exported here for existing importers and tests.
import { pickHighlights, MAX_HIGHLIGHTS, type Highlight } from "@/resume/highlights";
export { pickHighlights, MAX_HIGHLIGHTS };
...
const VOICE = [
  ...
  "Give before you ask. The recipient is a busy person doing you a favour by reading; they must come away feeling seen and respected, not processed. Structure, in this order: (1) one specific, true line about THEM that carries genuine appreciation — say what about their path, team, or work you actually find admirable, interesting, or worth learning from, tied to a concrete fact from the Recipient facts below (a real compliment about one particular thing is the warmest sentence in the message; a generic one that anyone could receive is the coldest); if no facts beyond a title and company are given, neither pretend familiarity nor announce that you know nothing about them — say, naturally, what about their team's or role's work draws you (widely known public facts about what the company does are fine here; nothing about the person beyond their title); (2) who you are in one clause, plus ONE concrete thing you have built or done that connects to their world, chosen from the candidate highlights below — a story, not a résumé dump or a list of skills; (3) the ask, small and explicit, with an easy out; (4) a warm close that stands on its own whatever they decide.",
  ...
  "Praise is welcome and important, but only specific, earned praise: compliment the particular thing (a post, a project, a career move, what their team ships) and say why it stands out to you — never the person in general. Never flatter generically ('impressive background', 'amazing work') and never use hollow openers ('I hope this finds you well', 'I'm reaching out because'). Specific beats effusive. Plain words; contractions are fine; at most one exclamation mark in the whole message; no emoji. Write in English.",
  "Tone: sincere, specific, and human. Always open with a greeting by first name ('Hi Jane,') — both the message and the note. Keep the message body to at most 120 words.",
  "Sound like a real person, not a system. Put one or two small human touches in every message, in your own words: acknowledge near the top that their time is valuable and that you'll keep this short and to the point (and then actually keep it short); thank them for reading this far at the end; or add one honest aside (that cold-messaging feels a bit awkward, that you're still working out where you fit). Vary which touches you use, their wording and their placement from message to message — never stack all of them, and never let a touch harden into a formula that every recipient gets.",
  ...
  for (let attempt = 0; attempt < 3 && note === null; attempt++) {
    const req: LlmRequest = {
      system:
        ...
        "keep the line that is specifically about the recipient — the specific compliment is what makes the note land — (and the shared-school opener if there is one), keep the company and at most one role by name, " +
