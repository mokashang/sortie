...
describe("give-first wording", () => {
  ...
  it("system prompt carries the give-before-ask structure, the easy out, and bans flattery/hollow openers", () => {
    ...
    expect(req.system).toMatch(/sound like a real person/i);
    expect(req.system).toMatch(/their time is valuable/i);
    expect(req.system).toMatch(/thank them for reading/i);
    expect(req.system).toMatch(/never let a touch harden into a formula/i);
  ...
  it("generateDraft feeds the experiences table and the person's notes into the prompt", async () => {
    ...
    createExperience(d, U, { kind: "skill", title: "Languages", bullets: [{ text: "C++, Python", directions: ["swe_general"] }], sort_order: 0 });
    const personId = upsertPerson(d, U, { name: "Jane", company: "Acme", relation: "engineer", linkedin_url: "in/jane", notes: "Leads the payments platform team; posted about idempotency last week." });
    const { backend, requests } = fakeBackend({ message: "Hi Jane", note: "Hi Jane" });
    await generateDraft(d, { userId: U, backend, profile: testProfile(), personId, playbook: "coffee_chat" });
  ...
  it("shortenNote's compressor keeps the line about them and never reduces the note to a bare referral ask", async () => {
    ...
    const id = createOutreach(d, U, { personId: pid, playbook: "referral", channel: "linkedin", draft: "Your path from USC to Google's SRE team caught my eye. I'm an MS ECE student; would love to hear how you found the team, and ask about the SRE opening if you're open to it." });
    ...
    const r = await shortenNote(d, { userId: U, backend, outreachId: id, max: 120 });
...
describe("generateDraft", () => {
  it("creates an outreach row from a fake backend's fixed JSON, with draft = message for linkedin", async () => {
    const d = db();
    const personId = upsertPerson(d, U, { name: "Jane Doe", relation: "engineer" });
    const { backend } = fakeBackend({ message: "Hi Jane, would love 15 min to chat!" });

    const result = await generateDraft(d, { userId: U, backend, profile: testProfile(), personId, playbook: "coffee_chat" });
  ...
  it("for email channel, prefixes the draft with 'Subject: ...' when the model returns a subject", async () => {
    const d = db();
    const personId = upsertPerson(d, U, { name: "Jane Doe", relation: "recruiter" });
    const { backend } = fakeBackend({ message: "Hi Jane, ...", subject: "Referral for SWE role" });

    const result = await generateDraft(d, { userId: U,
      backend,
      profile: testProfile(),
      personId,
      playbook: "recruiter",
  ...
  it("rejects an invalid playbook before calling the backend", async () => {
    ...
    await expect(
      generateDraft(d, { userId: U, backend, profile: testProfile(), personId, playbook: "cold_call" as never })
    ).rejects.toThrow();
  ...
  it("for a followup, includes the prior outreach's last thread_log entry in the prompt sent to the backend", async () => {
    const d = db();
    const personId = upsertPerson(d, U, { name: "Jane Doe", relation: "engineer" });
    const priorId = createOutreach(d, U, { personId, playbook: "coffee_chat", channel: "linkedin", draft: "hi" });
    ...
    await generateDraft(d, { userId: U, backend, profile: testProfile(), personId, playbook: "followup" });
  ...
  it("throws for an unknown personId", async () => {
    ...
    await expect(
      generateDraft(d, { userId: U, backend, profile: testProfile(), personId: 9999, playbook: "coffee_chat" })
    ).rejects.toThrow();
  });
});
...
describe("multi-job referral drafts", () => {
  ...
  it("generateDraft with jobIds creates one outreach linked to all jobs", async () => {
    ...
    const res = await generateDraft(d, { userId: U, backend, profile: testProfile(), personId: pid, playbook: "referral", jobIds: [j1, j2] });
...
describe("connection-note variant", () => {
  it("linkedin drafts ask the model for a <=280-char note and store it", async () => {
    ...
    const res = await generateDraft(d, { userId: U, backend, profile: testProfile(), personId: pid, playbook: "referral", jobIds: [j1], channel: "linkedin" });
    ...
    expect(listOutreach(d, U, { jobId: j1 })[0].draftNote).toBe(res.draftNote);
  });
  ...
  it("falls back to a sentence-trim when the note is missing or over the cap; email drafts get none", async () => {
    ...
    const res = await generateDraft(d, { userId: U, backend, profile: testProfile(), personId: pid, playbook: "referral", channel: "linkedin" });
    ...
    const er = await generateDraft(d, { userId: U, backend: eb, profile: testProfile(), personId: pid, playbook: "referral", channel: "email" });
...
describe("shortenNote", () => {
  ...
  it("re-compresses an approved (pending_send) note to the live cap without touching status", async () => {
    const d = db();
    const pid = upsertPerson(d, U, { name: "Jane", company: "Google" });
    const id = createOutreach(d, U, { personId: pid, playbook: "referral", channel: "linkedin", draft: "Hi Jane, fellow Trojan. Long approved text. Would you refer me?", draftNote: "x".repeat(258) });
    ...
    const r = await shortenNote(d, { userId: U, backend: s.backend, outreachId: id, max: 200 });
  ...
  it("falls back to a trim after three overlong attempts; refuses sent rows", async () => {
    const d = db();
    const pid = upsertPerson(d, U, { name: "Jane", company: "Google" });
    const id = createOutreach(d, U, { personId: pid, playbook: "referral", channel: "linkedin", draft: "First short sentence. " + "Second sentence that is long enough to matter here. ".repeat(5) });
    const s = seqBackend(["z".repeat(500)]);
    const r = await shortenNote(d, { userId: U, backend: s.backend, outreachId: id, max: 100 });
    ...
    await expect(shortenNote(d, { userId: U, backend: s.backend, outreachId: id, max: 100 })).rejects.toThrow(/pending_send/);
  });
});
