...
// Each account keeps its own folder: the owner stays on data/documents (where anything uploaded
// before accounts lives), every other account gets data/users/<id>/documents.
export function userDocumentsDir(db: DB, userId: string): string {
  return ownerId(db) === userId ? documentsDir() : path.join(userDataDir(userId), "documents");
}
