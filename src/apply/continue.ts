...
// Filled applications the user has not decided on yet — each one is an open Chrome tab waiting
// for a click on /apply. Approving or rejecting shrinks it; submitting does not change it.
export function unconfirmedCount(db: DB): number {
  return (
    db.prepare("SELECT COUNT(*) n FROM applications WHERE status = 'awaiting_confirm' AND confirm_decision IS NULL").get() as {
      n: number;
    }
  ).n;
}
  ...
  const rows = db
    .prepare(
      `SELECT m.direction, a.status, a.confirm_decision, a.needs_manual_reason, a.pending_questions
       FROM applications a LEFT JOIN matches m ON m.job_id = a.job_id WHERE a.run_id = ?`
    )
...
function insertPausedRun(db: DB, channel: ExecutorChannel, options: StartOptions, unconfirmed: number): number {
  const summary = `等待确认:${unconfirmed} 份填好的申请还没确认,降到 ${BACKLOG_RESUME_AT} 份以下自动继续`;
  const r = db
    .prepare("INSERT INTO executor_runs (kind, status, channel, pid, options, summary) VALUES ('apply', 'paused', ?, NULL, ?, ?)")
    .run(channel, JSON.stringify(options), summary);
...
// confirmation backlog, or ends the chain. Never throws — a bookkeeping failure must not turn
// a successful finish into an error.
export function maybeContinueApplyRun(db: DB, runId: number, deps: ContinueDeps = {}): ContinueResult {
  try {
    const run = db.prepare("SELECT kind, status, channel, options FROM executor_runs WHERE id = ?").get(runId) as
      | { kind: string; status: string; channel: string; options: string }
      | undefined;
    ...
    const unconfirmed = unconfirmedCount(db);
    const live = (deps.hasLiveOrQueuedRun ?? hasLiveOrQueuedRun)(db, "apply");
    if (unconfirmed >= BACKLOG_PAUSE_AT || live) {
      return { action: "paused", runId: insertPausedRun(db, channel, next, unconfirmed), unconfirmed };
    }
    const started = (deps.startExecutor ?? startExecutor)(db, "apply", next, deps.logDir ? { logDir: deps.logDir } : {}, channel);
...
// Called whenever the confirmation backlog may have shrunk (approve/reject on /apply) or a
// session freed up (any apply run finished): if a chain is parked as 'paused' and the backlog is
// low enough, hand it to a session. A user_chrome row simply becomes 'queued' (same id, so the
// card keeps its identity); a headless row spawns a fresh run and the placeholder is closed out.
export function resumePausedChainIfReady(db: DB, deps: ContinueDeps = {}): ContinueResult {
  try {
    const paused = db
      .prepare("SELECT id, channel, options, log_path FROM executor_runs WHERE kind = 'apply' AND status = 'paused' ORDER BY id DESC LIMIT 1")
      .get() as { id: number; channel: string; options: string; log_path: string | null } | undefined;
    ...
    if ((deps.hasLiveOrQueuedRun ?? hasLiveOrQueuedRun)(db, "apply")) return { action: "none", reason: "another apply run is live or queued" };
    ...
    const started = (deps.startExecutor ?? startExecutor)(db, "apply", options, deps.logDir ? { logDir: deps.logDir } : {}, "headless");
...
// The user started a new plan from /apply: whatever chain was parked waiting for confirmations
// is superseded — two chains competing for one session would only confuse the board.
export function supersedePausedChain(db: DB): number {
  return db
    .prepare("UPDATE executor_runs SET status = 'stopped', summary = '被新的投递计划取代', ended_at = datetime('now') WHERE kind = 'apply' AND status = 'paused'")
    .run().changes;
}
