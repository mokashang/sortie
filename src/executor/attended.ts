...
export function buildAttendedPrompt(
  ...
  return [
    ...
    `**自动投递(设置页的开关,用户可能开着)**:看每次 POST /api/apply/report 的响应。回报 awaiting_confirm 的响应里若有 \`autoApproved: true\`,说明 App 已替用户批准——不要停下等消息,立刻在同一个标签页重读表单核对与 filledFields 一致后点 Submit,看到成功页 POST report {jobId,status:'submitted'}(响应里没有 autoApproved 就照常停下等 [Sortie] approved)。回报 needs_info 的响应里若有 \`autoAnswered: true\`,\`infoAnswers\` 就是 App 按用户档案替用户答好的答案——不要取下一个,立刻填进这个标签页、回读、回报 awaiting_confirm;\`autoAnswered: "partial"\` 或 false 则 \`remaining\` 里的项目仍在等用户,照常取下一个。内推同理:POST /api/referral/outreach 的响应里若有 \`autoApproved: true\`(status 已是 pending_send),不要轮询 /api/referral/pending,按 §3.10.d 立刻发送(仍看对话框里的字数上限与本月剩余邀请数,超限就 shorten / 跳过)。开关开着时只有 登录 / 创建账号 / 验证码 / 缺文件 / 必须亲自完成 的事才会等用户;不要因为开关开着就少问——该报 needs_info 的照报,App 会自己判断能不能替用户答。`,
