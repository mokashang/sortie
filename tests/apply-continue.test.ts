...
describe("接力 maybeContinueApplyRun / resumePausedChainIfReady", () => {
  ...
  function startSegment(options: StartOptions): number {
    const { id } = startExecutor(db, U, "apply", options, { logDir }, "user_chrome");
    expect(claimNextRun(db, U, "user_chrome")?.id).toBe(id);
    return id;
  }
  function fillNext(direction = "swe_general"): number {
    const task = takeNextApplication(db, U, profile, { direction }) as { jobId: number };
    expect(task.jobId).toBeTypeOf("number");
    reportFill(db, U, { jobId: task.jobId, status: "awaiting_confirm", filledFields: { Name: "Mengjia Shang" } });
    return task.jobId;
  }
  ...
  it("queues the next segment with the remaining plan, chain bookkeeping and resume:true; the last segment reports cumulative progress", () => {
    ...
    fillNext();
    fillNext();
    finishRun(db, U, a, "done", "segment 1");
    ...
    fillNext();
    fillNext();
    finishRun(db, U, b, "done", "segment 2");
  ...
  it("drops an entry the segment attempted without progress (quota, exhausted) and keeps entries it never reached", () => {
    ...
    const task = takeNextReferral(db, U, { direction: "swe_general" }) as { company: string; jobs: { jobId: number }[] };
    expect(task.company).toBe("Tradeweb");
    reportNoContact(db, U, task.jobs.map((j) => j.jobId), "LinkedIn free invite quota exhausted");
    fillNext();
  ...
  it("parks the next segment as paused behind the confirmation backlog and resumes it in place once the user decides enough", () => {
    ...
    finishRun(db, U, a, "done");
    expect(unconfirmedCount(db, U)).toBe(10);
    ...
    expect(resumePausedChainIfReady(db, U, { logDir })).toMatchObject({ action: "none", reason: expect.stringContaining("backlog") });

    // Four decisions bring the backlog to 6: still parked. Approve and reject both count.
    decide(db, U, filled[0], "approve");
    decide(db, U, filled[1], "reject", "wrong resume");
    decide(db, U, filled[2], "approve");
    decide(db, U, filled[3], "approve");
    expect(unconfirmedCount(db, U)).toBe(6);
    expect(resumePausedChainIfReady(db, U, { logDir })).toMatchObject({ action: "none" });
    ...
    const result = decideAndMaybeAutoStart(db, U, filled[4], "reject", "not interested", { logDir });
    ...
    expect(claimNextRun(db, U, "user_chrome")?.id).toBe(p);
  });
  ...
  it("gives a zero-progress segment one more try (the entry may never have been reached), then stops the chain", () => {
    ...
    expect(takeNextApplication(db, U, profile, { direction: "swe_general" })).toEqual({ done: true });
    finishRun(db, U, a, "done", "queue empty");
    const r = maybeContinueApplyRun(db, a, { logDir }) as { action: string; runId: number };
    expect(r.action).toBe("queued");
    expect(optionsOf(db, r.runId).chain).toMatchObject({ step: 2, zeroRuns: 1 });

    expect(claimNextRun(db, U, "user_chrome")?.id).toBe(r.runId);
    finishRun(db, U, r.runId, "done", "still empty");
