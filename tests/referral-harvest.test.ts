...
function sentOutreach(db: DB, name = "Jane"): number {
  const pid = upsertPerson(db, U, { name, company: "Google", relation: "alum", linkedin_url: `https://li/${name}` });
  const id = createOutreach(db, U, { personId: pid, playbook: "referral", channel: "linkedin", draft: "Hi, refer me?", jobIds: [seedJob(db)] });
  approveOutreach(db, U, id);
  reportSent(db, U, id, "Hi, refer me?");
  return id;
}
...
describe("harvestOutreach", () => {
  it("accepted invite without messages → status accepted, stage accepted, no LLM call", async () => {
    ...
    const r = await harvestOutreach(db, { userId: U, backend: b, outreachId: id, accepted: true });
  ...
  it("nothing changed → still sent/pending", async () => {
    const db = openDb(":memory:");
    const id = sentOutreach(db);
    const r = await harvestOutreach(db, { userId: U, backend: stageBackend({}), outreachId: id, accepted: false });
  ...
  it("received messages → replied, thread merged (deduped), Claude stage stored; referred link surfaces", async () => {
    ...
    const r = await harvestOutreach(db, { userId: U,
      backend: b,
      outreachId: id,
      accepted: true,
      messages: [
        { dir: "sent", text: "Hi, refer me?" }, // duplicate of the sent text already in thread_log
        // Stamped one minute after the sent entry (which reportSent stamps with "now") so the merged
        // thread order stays sent → received on any date the suite runs.
    ...
    const r2 = await harvestOutreach(db, { userId: U, backend: b, outreachId: id, messages: [{ dir: "received", text: "Sure! Submitted: https://g/ref/123" }] });
  ...
  it("classifier failure keeps replied status and falls back to the raw tail", async () => {
    ...
    const r = await harvestOutreach(db, { userId: U, backend: bad, outreachId: id, messages: [{ dir: "received", text: "Let me check with my manager." }] });
  ...
  it("refuses rows that never went out", async () => {
    const db = openDb(":memory:");
    const pid = upsertPerson(db, U, { name: "X", company: "Google" });
    const id = createOutreach(db, U, { personId: pid, playbook: "referral", channel: "linkedin", draft: "d" });
    await expect(harvestOutreach(db, { userId: U, backend: stageBackend({}), outreachId: id })).rejects.toThrow(/sent\/accepted\/replied/);
  });
...
describe("referralChecklist + enqueueReferralCheck + claim kinds", () => {
  it("lists only job-linked linkedin outreach that went out; enqueues one attended run at a time", () => {
    ...
    const pid = upsertPerson(db, U, { name: "Coffee", company: "Google" });
    const coffee = createOutreach(db, U, { personId: pid, playbook: "coffee_chat", channel: "linkedin", draft: "c" });
    approveOutreach(db, U, coffee);
    reportSent(db, U, coffee);
    expect(referralChecklist(db, U).map((c) => c.outreachId)).toEqual([a]);
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "rc-"));
    const start = vi.fn((d: DB, u: string, kind: string, o: object, deps: object, ch: string) => startExecutor(d, u, kind as "referral_check", o, { logDir: tmp }, ch as "user_chrome"));
    const id1 = enqueueReferralCheck(db, U, { startExecutor: start as never });
    expect(id1).toBeTruthy();
    expect(enqueueReferralCheck(db, U, { startExecutor: start as never })).toBeNull(); // already queued
    // A claimer that only knows apply/referral_check takes it; one restricted to 'scan' does not.
    expect(claimNextRun(db, U, "user_chrome", ["scan"])).toBeNull();
    expect(claimNextRun(db, U, "user_chrome", ["apply", "referral_check"])?.kind).toBe("referral_check");
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("empty checklist → nothing enqueued; headless referral_check refused", () => {
    const db = openDb(":memory:");
    expect(enqueueReferralCheck(db, U, { startExecutor: vi.fn() as never })).toBeNull();
    expect(() => startExecutor(db, U, "referral_check", {}, { spawn: vi.fn() as never, logDir: os.tmpdir() }, "headless")).toThrow(/attended-only/);
  });
